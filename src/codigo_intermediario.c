...
const char* get_decl_kind_as_string(DeclarationKind kind) {
    switch (kind) {
        case var_k: return "var";
        case fun_k: return "function";
        case param_k: return "param";
        case array_k: return "array";
...
char *percorrer_arvore(No *node_tree, Tac **tac_list_ptr, HashTable *symbol_table, int expression_parametro, int esq_assign) {
        ...
        case statement_k: {
            ...
            switch (node_tree->kind_union.stmt) {
                case if_k: {
                    ...
                    char *cond_res = percorrer_arvore(node_tree->filho[0], tac_list_ptr, symbol_table, 0, 0);
                    ...
                    res_child = percorrer_arvore(node_tree->filho[1], tac_list_ptr, symbol_table, 0, 0);
                    free(res_child); 
                    ...
                    *tac_list_ptr = criarNoTac(*tac_list_ptr, LAB, label_else, "", "");
                    ...
                    if (node_tree->filho[2]) {
                        res_child = percorrer_arvore(node_tree->filho[2], tac_list_ptr, symbol_table, 0, 0);
                        free(res_child); 
                    }
                ...
                case return_k:
                    if (node_tree->filho[0]) {
                        char *ret_val = percorrer_arvore(node_tree->filho[0], tac_list_ptr, symbol_table, 0, 0);
                        if (ret_val) {
                    ...
                    break;
                default:
                    res_child = percorrer_arvore(node_tree->filho[0], tac_list_ptr, symbol_table, 0, 0);
                    free(res_child);
                    result_str = NULL;
                    break;
            }
            ...
            switch (node_tree->kind_union.expr) {
                ...
                case id_k: {
                    ...
                    if (tmp != NULL) {
                        ...
                        if (expression_parametro){
                            Symbol *simbolo = find_symbol(symbol_table, node_tree->lexmema, escopo);
                            if (simbolo == NULL) {
                                simbolo = find_symbol(symbol_table, node_tree->lexmema, "GLOBAL");
                            }

                            if (simbolo != NULL) {
                                const char* tipo_string = get_decl_kind_as_string(simbolo->id_type);
                                *tac_list_ptr = criarNoTac(*tac_list_ptr, PARAM, tmp, tipo_string, "");
                            }
                            else {
                                // Números inteiros
                                *tac_list_ptr = criarNoTac(*tac_list_ptr, PARAM, tmp, "var", "");
                            }
                        }
                ...
                case arr_k: {
                    char *index_res = percorrer_arvore(node_tree->filho[0], tac_list_ptr, symbol_table, 0, esq_assign);
                    ...
                    if (index_res) {
                        ...
                        if (expression_parametro){
                            //Acesso índice do array
                            *tac_list_ptr = criarNoTac(*tac_list_ptr, PARAM, result_str, "var", "");
                        }
                        free(index_res);
                    } else {
                ...
                case fun_k:
                    ...
                    for (int i = 0; i < NUMMAXFILHOS; i++) {
                        if (node_tree->filho[i]) {
                            res_child = percorrer_arvore(node_tree->filho[i], tac_list_ptr, symbol_table, 0, 0);
                            free(res_child);
                        }
                    }
                    if(strcmp(node_tree->lexmema, "int") !=0 && strcmp(node_tree->lexmema, "void") != 0){
    ...
    if(node_tree->irmao) {
        char *sibling_res = percorrer_arvore(node_tree->irmao, tac_list_ptr, symbol_table, expression_parametro, 0);
        free(sibling_res);
    }
